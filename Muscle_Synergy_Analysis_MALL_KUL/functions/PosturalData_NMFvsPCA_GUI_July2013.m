...
global DATAsrc DATA_LST checkwalk time nmus ncond ntrials nbins color file path_out
...
if checkwalk==1
%     load Synergy_Data_L.mat DATAsrc DATA_LST time %eventually modify this to allow user to select a file
    [file,path] = uigetfile('*.mat', 'Select Synergy Data', cd);
    load (fullfile(path,file))
    str1=char({'Right', 'Left'});
    if file(14)== 'R'
        
        path_out = (fullfile(cd, ['Output_Matrix_NMF_' str1(1,:) '_S' subjectnr]));
        
        if ~exist(path_out, 'dir')
            mkdir(path_out)
        end
        save(fullfile(path_out, 'Synergy_Data_R.mat'), 'DATA_LST', 'DATAsrc', 'MuscleNames', 'time', 'subjectnr')
    elseif file(14)== 'L'
        
        path_out = (fullfile(cd, ['Output_Matrix_NMF_' str1(2,1:4) '_S' subjectnr]));
        
        if ~exist(path_out, 'dir')
            mkdir(path_out)
        end      
        save(fullfile(path_out, 'Synergy_Data_L.mat'), 'DATA_LST', 'DATAsrc', 'MuscleNames', 'time', 'subjectnr')
    end
    save(fullfile(cd,'path_out'), "path_out")
    
else
    load Synergy_Data_R.mat DATAsrc DATA_LST
    nmus=20; ncond=12; ntrials=5; nbins=4;
...
for i=1:nmus
    ...
    elseif checkwalk==1 %Plot entire trial for walking data
        subplot(10,2,i) %to plot only one time bin
        plot(time,DATAsrc(i,:));
        ylim([0 1])
        xlim([0 time(end)])
        ylabel(DATA_LST(i,:))
...
% prompt = {'What side did you analyze? Pick: Left or Right'};
% name = char({'Chose correct name extension for output directory'});
% defaultanswer = {'Left'};
% windowsize = [1 80];
% options.WindowStyle = 'modal';
% options.Resize = 'on';
% answer = inputdlg(prompt,name,windowsize,defaultanswer,options);
% str1 = char(answer);

% assignin('base',"synergy_nmf",synergy_nmf)
        
% path_out = (fullfile(cd, ['Output_Matrix_NMF_' str1(1,:)]));
% 
load path_out.mat
save(fullfile(path_out, 'synergy_nmf'), 'synergy_nmf')
delete path_out.mat
...
[COEFF,SCORE,pcvars] = pca(DATAsrc','econ');
