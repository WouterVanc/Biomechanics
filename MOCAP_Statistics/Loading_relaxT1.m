...
% Results directories
path_out = fullfile(cd,'Results_load_relaxT1');
...
for y = [2,4,5]
    for q = 1:length(loadvars_perpeak) % loading variables per peak
        ...
        RTresults.(side{y}).T1.OA
        
        %%%%% OA %%%%%
        % peak 1
        [r,pval] = corr(loading_results.oa.p1.(loadvars_perpeak{q}), RTresults.(side{y}).T1.OA,'Type','Spearman', 'tail','both');
        coeffmatrix_oa_p1(q,c) = r;
        pvalmatrix_oa_p1(q,c) = pval;
        % peak 2
        [r2,pval2] = corr(loading_results.oa.p2.(loadvars_perpeak{q}), RTresults.(side{y}).T1.OA,'Type','Spearman', 'tail','both');
        coeffmatrix_oa_p2(q,c) = r2;
        pvalmatrix_oa_p2(q,c) = pval2;

        %%%%% CO %%%%%
        [r3,pval3] = corr(loading_results.co.p1.(loadvars_perpeak{q}), RTresults.(side{y}).T1.CO,'Type','Spearman', 'tail','both');
        coeffmatrix_p1(q,c) = r3;
        pvalmatrix_p1(q,c) = pval3;
        % peak 2
        [r4,pval4] = corr(loading_results.co.p2.(loadvars_perpeak{q}), RTresults.(side{y}).T1.CO,'Type','Spearman', 'tail','both');
        ...
        x_mean = mean(RTresults.(side{y}).T1.OA);
        y_std = std(loading_results.oa.p1.(loadvars_perpeak{q})/factor);
        x_std = std(RTresults.(side{y}).T1.OA);
        slope = r*y_std/x_std;
        yfit = y_mean + slope*(RTresults.(side{y}).T1.OA - x_mean);
        
        % CO
        y_mean2 = mean(loading_results.co.p1.(loadvars_perpeak{q})/factor);
        x_mean2 = mean(RTresults.(side{y}).T1.CO);
        y_std2 = std(loading_results.co.p1.(loadvars_perpeak{q})/factor);
        x_std2 = std(RTresults.(side{y}).T1.CO);
        slope2 = r3*y_std2/x_std2;
        yfit2 = y_mean2 + slope2*(RTresults.(side{y}).T1.CO - x_mean2);
        
        f1 = figure;
        hold on;
        s = scatter(RTresults.(side{y}).T1.OA, loading_results.oa.p1.(loadvars_perpeak{q})/factor,'rsq','filled');
        p = plot(RTresults.(side{y}).T1.OA, yfit, 'color', 'r', 'LineWidth', 1, 'DisplayName', 'OA');
        s2 = scatter(RTresults.(side{y}).T1.CO, loading_results.co.p1.(loadvars_perpeak{q})/factor,'bsq','filled');
        p2 = plot(RTresults.(side{y}).T1.CO, yfit2, 'color', 'b', 'LineWidth', 1, 'DisplayName','CO');
        
        title('Peak 1')
        xlabel([prefix ' T1\rho relaxation time [ms]'])
        ...
        f2 = figure;
        hold on;
        s = scatter(RTresults.(side{y}).T1.OA, loading_results.oa.p2.(loadvars_perpeak{q})/factor,'rsq','filled');
        p = plot(RTresults.(side{y}).T1.OA, yfit, 'color', 'r', 'LineWidth', 1, 'DisplayName', 'OA');
        s2 = scatter(RTresults.(side{y}).T1.CO, loading_results.co.p2.(loadvars_perpeak{q})/factor,'bsq','filled');
        p2 = plot(RTresults.(side{y}).T1.CO, yfit2, 'color', 'b', 'LineWidth', 1, 'DisplayName','CO');
...
for y = [2,4,5]
    for q = 1:length(loadvars_imp) % loading variables impulse
        ...
        %%%%% OA %%%%%
        % Impulse 
        [r5,pval5] = corr(loading_results.oa.imp.(loadvars_imp{q}), RTresults.(side{y}).T1.OA,'Type','Spearman', 'tail','both');
        coeffmatrix_oa_imp(q,c) = r5;
        pvalmatrix_oa_imp(q,c) = pval5;

        %%%%% CO %%%%%
        [r6,pval6] = corr(loading_results.co.imp.(loadvars_imp{q}), RTresults.(side{y}).T1.CO,'Type','Spearman', 'tail','both');
        ...
        f1 = figure;
        hold on;
        s = scatter(RTresults.(side{y}).T1.OA, loading_results.oa.imp.(loadvars_imp{q}),'rsq','filled');
        p = plot(RTresults.(side{y}).T1.OA, yfit, 'color', 'r', 'LineWidth', 1, 'DisplayName', 'OA');
        s2 = scatter(RTresults.(side{y}).T1.CO, loading_results.co.imp.(loadvars_imp{q}),'bsq','filled');
        p2 = plot(RTresults.(side{y}).T1.CO, yfit2, 'color', 'b', 'LineWidth', 1, 'DisplayName','CO');
...
for y = [2,4,5]   
    for q = 1:length(loadvars_stance) % loading variables stance
        ...
        %%%% OA %%%%%
        [r7,pval7] = corr(loading_results.oa.stance.(loadvars_stance{q}), RTresults.(side{y}).T1.OA,'Type','Spearman', 'tail','both');
        coeffmatrix_oa_stance(q,c) = r7;
        pvalmatrix_oa_stance(q,c) = pval7;

        %%%% CO %%%%%            
        [r8,pval8] = corr(loading_results.co.stance.(loadvars_stance{q}), RTresults.(side{y}).T1.CO,'Type','Spearman', 'tail','both');
        ...
        f1 = figure;
        hold on;
        s = scatter(RTresults.(side{y}).T1.OA, loading_results.oa.stance.(loadvars_stance{q})/1000000,'rsq','filled');
        p = plot(RTresults.(side{y}).T1.OA, yfit, 'color', 'r', 'LineWidth', 1, 'DisplayName', 'OA');
        s2 = scatter(RTresults.(side{y}).T1.CO, loading_results.co.stance.(loadvars_stance{q})/1000000,'bsq','filled');
        p2 = plot(RTresults.(side{y}).T1.CO, yfit2, 'color', 'b', 'LineWidth', 1, 'DisplayName','CO');
...
filename = 'Loading_x_relaxT1.xlsx';
