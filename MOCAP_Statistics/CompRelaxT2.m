...
% Results directories
path_out = fullfile(cd,'Results_relax_T2_Comp');
...
for q = 1:length(side) 
    % Peak 1
    mean_oa = mean(RTresults.(side{q}).T2.OA);
    std_oa = std(RTresults.(side{q}).T2.OA);
    mean_co = mean(RTresults.(side{q}).T2.CO);
    std_co = std(RTresults.(side{q}).T2.CO);
    ...
    [p,h] = ranksum(RTresults.(side{q}).T2.OA,RTresults.(side{q}).T2.CO);
    ...
    % Combine data
    y(:,c) = RTresults.(side{q}).T2.OA;
    y_temp = RTresults.(side{q}).T2.CO;
    y_temp(6:10,1) = NaN;
    y(:,c2) = y_temp;   

    hh = [hh; RTresults.(side{q}).T2.OA];
    h2 = [h2; y_temp];
end
h3 = [hh;h2];
...
legend('Control', 'OA', Location='northwest')

plotname = (['Relaxation times T2' '.png']);
