...
TEfiles = (['E:\Biomedical Research in Movement Sciences\Master Thesis\Masterproef_Data\MRI\Mapping\OA\MRI_S' num2str(subject) '\ET_files_s' num2str(subject)]);
directory_mask = (['E:\Biomedical Research in Movement Sciences\Master Thesis\Masterproef_Data\MRI\Mapping\FULL_NON_Weightbearing_zones\Masks_NONFWZ\T2\S' num2str(subject)]);
contentM = dir(directory_mask);


path_mask = fullfile(cd,['MATLAB_masksT2_FNWBZ_s' num2str(subject)]); 
...
%% write results
path_out = fullfile(cd,['Processed_FNWBZ_s' num2str(subject)]); 
...
path_out_f = fullfile(cd,['Filtered_FNWBZ_s' num2str(subject)]); 
...
save(fullfile(cd,['T2_FNWBZ_S' num2str(subject)]), 'MeanRT','stdRT')
