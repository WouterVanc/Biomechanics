...
def Dynamic_AFM(path,staticinfo,gc,footside,forefootanalysis,trialname,cond):
    # load in data
    c_dyn = c3d(path)
    
    # Extract variables 
    point_data = c_dyn['data']['points']
    ...
    # Slice gaitcycle out of data
    point_data_dyn = point_data[:,:,gc[0]:gc[1]]

    # Adjust markers if necesary 
    if footside==1:
        labels_dyn = adjust_marker_names_R(labels_dyn)
    if footside==2:
        labels_dyn = adjust_marker_names_L(labels_dyn)
    ...
    point_data_dyn = nanfilter(point_data_dyn, framerate_dyn)
    ...
    if footside==1: # Right foot
        gaitcycle['HFSKx'] = a_hfsk
        gaitcycle['HFSKy'] = b_hfsk
        gaitcycle['HFSKz'] = c_hfsk
        gaitcycle['FFHFx'] = a_ffhf
        gaitcycle['FFHFy'] = b_ffhf
        gaitcycle['FFHFz'] = c_ffhf
        gaitcycle['FFSKx'] = a_ffsk
        gaitcycle['FFSKy'] = b_ffsk
        gaitcycle['FFSKz'] = c_ffsk
        gaitcycle['MFHFx'] = a_mfhf
        gaitcycle['MFHFy'] = b_mfhf
        gaitcycle['MFHFz'] = c_mfhf
        gaitcycle['FFMFx'] = a_ffmf
        gaitcycle['FFMFy'] = b_ffmf
        gaitcycle['FFMFz'] = c_ffmf
        gaitcycle['FFmMFx'] = a_ffMmf
        gaitcycle['FFmMFy'] = b_ffMmf
        gaitcycle['FFmMFz'] = c_ffMmf
        gaitcycle['FFlMFx'] = a_ffLmf
        gaitcycle['FFlMFy'] = b_ffLmf
        gaitcycle['FFlMFz'] = c_ffLmf
        gaitcycle['HXFFx'] = a_hxff
        gaitcycle['HXFFy'] = b_hxff
        gaitcycle['HXFFz'] = c_hxff
        gaitcycle['HXFFMx'] = a_hxffm
        gaitcycle['HXFFMy'] = b_hxffm
        gaitcycle['HXFFMz'] = c_hxffm
        gaitcycle['FOSKx'] = a_fosk
        gaitcycle['FOSKy'] = b_fosk
        gaitcycle['FOSKz'] = c_fosk
        gaitcycle['MLA'] = mla
        gaitcycle['TTA'] = tta
          
    if footside==2:
        gaitcycle['HFSKx'] = -(np.asarray(a_hfsk))
        gaitcycle['HFSKy'] = -(np.asarray(b_hfsk))
        gaitcycle['HFSKz'] = (np.asarray(c_hfsk))
        gaitcycle['FFHFx'] = -(np.asarray(a_ffhf))
        gaitcycle['FFHFy'] = -(np.asarray(b_ffhf))
        gaitcycle['FFHFz'] = (np.asarray(c_ffhf))
        gaitcycle['FFSKx'] = -(np.asarray(a_ffsk))
        gaitcycle['FFSKy'] = -(np.asarray(b_ffsk))
        gaitcycle['FFSKz'] = (np.asarray(c_ffsk))
        gaitcycle['MFHFx'] = -(np.asarray(a_mfhf))
        gaitcycle['MFHFy'] = -(np.asarray(b_mfhf))
        gaitcycle['MFHFz'] = (np.asarray(c_mfhf))
        gaitcycle['FFMFx'] = -(np.asarray(a_ffmf))
        gaitcycle['FFMFy'] = -(np.asarray(b_ffmf))
        gaitcycle['FFMFz'] = (np.asarray(c_ffmf))
        gaitcycle['FFmMFx'] = -(np.asarray(a_ffMmf))
        gaitcycle['FFmMFy'] = -(np.asarray(b_ffMmf))
        gaitcycle['FFmMFz'] = (np.asarray(c_ffMmf))
        gaitcycle['FFlMFx'] = -(np.asarray(a_ffLmf))
        gaitcycle['FFlMFy'] = -(np.asarray(b_ffLmf))
        gaitcycle['FFlMFz'] = (np.asarray(c_ffLmf))
        gaitcycle['HXFFx'] = -(np.asarray(a_hxff))
        gaitcycle['HXFFy'] = -(np.asarray(b_hxff))
        gaitcycle['HXFFz'] = (np.asarray(c_hxff))
        gaitcycle['HXFFMx'] = -(np.asarray(a_hxffm))
        gaitcycle['HXFFMy'] = -(np.asarray(b_hxffm))
        gaitcycle['HXFFMz'] = (np.asarray(c_hxffm))
        gaitcycle['FOSKx'] = -(np.asarray(a_fosk))
        gaitcycle['FOSKy'] = -(np.asarray(b_fosk))
        gaitcycle['FOSKz'] = (np.asarray(c_fosk))
        gaitcycle['MLA'] = mla
        gaitcycle['TTA'] = tta

    # Normalize gaitcycle to 101 datapoints
    gaitcycle_normalized = dict()
    gaitcycle_nonan = {key: [x for x in value if not isnan(x)] for (key,value) in gaitcycle.items()}
    for key in gaitcycle_nonan:
        gaitcycle_normalized[key] = norm_to_101(gaitcycle_nonan[key])

    # Calculate ROM S
    gaitcycle_ROM = dict()
    for key in gaitcycle:
        gaitcycle_ROM[key + 'min'] = min(gaitcycle[key]) 
        gaitcycle_ROM[key + 'max'] = max(gaitcycle[key])
    ...
    Dynamic_data['trial_info'] = (cond + '_' + trialname)
